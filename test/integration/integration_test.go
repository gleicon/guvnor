...
func (s *IntegrationTestSuite) SetupSuite() {
	...
	s.guvnorPort = testutils.FindFreePort(s.T())
...
func (s *IntegrationTestSuite) TearDownSuite() {
	...
	// Stop guvnor server
	if s.guvnorCmd != nil && s.guvnorCmd.Process != nil {
		s.guvnorCmd.Process.Kill()
		s.guvnorCmd.Wait()
	}
...
func (s *IntegrationTestSuite) createguvnorConfig() {
	config := fmt.Sprintf(`
server:
  port: %d
  host: "127.0.0.1"
...
`,
		...
		s.testConfig.TempDir,
		s.guvnorPort+1000, // Metrics port
	)

	s.testConfig.CreateTestConfig(s.T(), config)
}

func (s *IntegrationTestSuite) startguvnorServer() {
	...
	// Start guvnor server
	s.guvnorCmd = exec.CommandContext(s.ctx, "go", "run",
		"../../cmd/guvnor/main.go",
		"--config", s.testConfig.ConfigFile)

	s.guvnorCmd.Dir = filepath.Join(s.testConfig.TempDir, "../..")
	s.guvnorCmd.Stdout = os.Stdout
	s.guvnorCmd.Stderr = os.Stderr

	require.NoError(s.T(), s.guvnorCmd.Start())
}

func (s *IntegrationTestSuite) waitForServerReady() {
	// Wait for guvnor server to be ready
	err := testutils.WaitForPort("127.0.0.1", s.guvnorPort, 30*time.Second)
	require.NoError(s.T(), err, "guvnor server should start within 30 seconds")

	// Wait for apps to be started by guvnor
...
func (s *IntegrationTestSuite) TestBasicProxyFunctionality() {
	...
	for _, tt := range tests {
		s.T().Run(fmt.Sprintf("%s%s", tt.domain, tt.path), func(t *testing.T) {
			req, err := http.NewRequest("GET", fmt.Sprintf("https://127.0.0.1:%d%s", s.guvnorPort, tt.path), nil)
...
func (s *IntegrationTestSuite) TestTLSTermination() {
	...
	req, err := http.NewRequest("GET", fmt.Sprintf("https://127.0.0.1:%d/", s.guvnorPort), nil)
...
func (s *IntegrationTestSuite) TestHealthChecking() {
	...
	client := &http.Client{}
	resp, err := client.Get(fmt.Sprintf("http://127.0.0.1:%d/metrics", s.guvnorPort+1000))
	require.NoError(s.T(), err)
	defer resp.Body.Close()
	...
	assert.Contains(s.T(), metrics, "guvnor_health_checks_total")
	assert.Contains(s.T(), metrics, "guvnor_healthy_services")
}
...
func (s *IntegrationTestSuite) TestProcessRestart() {
	...
	// Find and kill the process (this would be handled by guvnor's process manager)
	killCmd := exec.Command("pkill", "-f", "node-server.js")
	killCmd.Run() // Ignore errors, process might not exist

	// Wait for guvnor to detect the crash and restart
...
func (s *IntegrationTestSuite) TestMetricsCollection() {
	...
	resp, err := metricsClient.Get(fmt.Sprintf("http://127.0.0.1:%d/metrics", s.guvnorPort+1000))
	require.NoError(s.T(), err)
	defer resp.Body.Close()
	...
	assert.Contains(s.T(), metrics, "guvnor_requests_total")
	assert.Contains(s.T(), metrics, "guvnor_request_duration")
	assert.Contains(s.T(), metrics, "guvnor_active_processes")
}
...
logging:
  level: "debug"`, -1)
	...
	// Send reload signal (this would depend on how guvnor handles config reloads)
	...
	client := &http.Client{}
	resp, err := client.Get(fmt.Sprintf("http://127.0.0.1:%d/status", s.guvnorPort+1000))
	if err == nil {
		defer resp.Body.Close()
